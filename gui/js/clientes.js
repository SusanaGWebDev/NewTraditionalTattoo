...
var listaPersonas =
[
    ['305160872','Susana', 'Gonzalez', '02-04-1999', '10:00','63069007','Cliente'],
    ['302860791','Gerardo', 'Molina', '13-08-1986', '16:00','86109530','Cliente']
];
...
function EliminarPersona(){
    ...
    for (var i = 0; i < listaPersonas.length; i++) {
        if (this.id == listaPersonas[i][0]) {
         persona = listaPersonas[i]; //obtiene los datos del cliente seleccionado
        }
    }

    var index = listaPersonas.indexOf(persona); //obtiene la posición de los datos del cliente seleccionado

    if (index > -1) {
        listaPersonas.splice(index, 1); //elimina al cliente
    }
...
function EditarPersona(IdPersona){
    ...
    if(sCedula==''|| sNombre == '' || sApellidos == '' || sFechaNac == '' || sTelefono == '' || sRol == ''){
        ...
        for (var i = 0; i < ListaPersonas.length; i++) {
            if (IdPersona == ListaPersonas[i][0]) {
                personaOriginal = ListaPersonas[i]; //obtiene los datos del cliente seleccionado
            }
        }
    
        var index = ListaPersonas.indexOf(personaOriginal); //obtiene la posición de los datos del cliente seleccionado
    
        if (index > -1) {
            ListaPersonas[index] = persona; //elimina al cliente
        }
